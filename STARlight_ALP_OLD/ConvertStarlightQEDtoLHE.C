...
//
// Run with "./convert X Y", where X is the STARlight output file to conver number and Y is the index
// of the first output file to be produced (an offset).

// Set those variables:
const unsigned int eventsPerFile = 10000; // N_events
...
{
  if(argc != 3){
    cout<<"usage: convert in_file_number first_out_file_number"<<endl;
    exit(0);
  }
  
  long long int dirNumber = atoi(argv[1]);
  cout<<"dir number:"<<dirNumber<<endl;
  
  string filename = "starlight/slight_"+to_string(dirNumber)+".out";
  ifstream infile(filename.c_str());
  if (! infile.is_open()) { cout << "\t ERROR: I can not open \"" << filename << "\"" << endl; return 1; }
...
//  getline(infile,temp_string); // The very first line is useless
  ...
  int currentFileNumber = atoi(argv[2]);
  cout<<"First output file number:"<<currentFileNumber<<endl;
  ...
  while(getline(infile,temp_string)){
    ...
    if(firstFile){
      sprintf(outfilename,"lhe/qed_ee_pbpb_%05d.lhe",currentFileNumber);
