...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   hltTreeReaderData(TTree *tree=0);
   virtual ~hltTreeReaderData();
...
#ifdef hltTreeReaderData_cxx
hltTreeReaderData::hltTreeReaderData(TTree *tree) : fChain(0) 
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      TDirectory * dir = (TDirectory*)f->Get("/eos/cms/store/group/phys_diffraction/diphoton/aug_reco_data_check_for_lumi/hiforest2/00/hiforest_0.root:/hltanalysis");
...
hltTreeReaderData::~hltTreeReaderData()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t hltTreeReaderData::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t hltTreeReaderData::LoadTree(Long64_t entry)
{
...
void hltTreeReaderData::Init(TTree *tree)
{
...
Bool_t hltTreeReaderData::Notify()
{
...
void hltTreeReaderData::Show(Long64_t entry)
{
...
Int_t hltTreeReaderData::Cut(Long64_t entry)
{
...
#endif // #ifdef hltTreeReaderData_cxx
