...
void plotQED_SS(TString algo="GED", double acop_cut=0.01, double luminosity=363.959, double mass_cut=5) {
   TFile *fdata = TFile::Open("outputDataAll_nochg.root");
   TTree *trdata = (TTree*) fdata->Get("tr" + algo);
   // TFile *fMC = TFile::Open("outputMCAll.root");
   TFile *fMC = TFile::Open("outputMCAll_nochg.root");
   ...
   trdata->Project(hmass_data->GetName(),"mass",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut));
   trdata->Project(hdeltapt_data->GetName(),"deltapt",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut));
   trdata->Project(hrap_data->GetName(),"rap",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut));
   trdata->Project(hpt_data->GetName(),"pt",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut));
   trdata->Project(hacop_data->GetName(),"acop",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut));
   TH1D *hmass_MC = SFuncert(trMC,"hmass_MC","mass",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut),50,0,100);
   TH1D *hdeltapt_MC = SFuncert(trMC,"hdeltapt_MC","deltapt",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut),10,0,1);
   TH1D *hrap_MC = SFuncert(trMC,"hrap_MC","rap",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut),25,-2.5,2.5);
   TH1D *hpt_MC = SFuncert(trMC,"hpt_MC","pt",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut),20,0,1);
   TH1D *hacop_MC = SFuncert(trMC,"hacop_MC","acop",Form("doubleEG2&&opp_chg!=0&&acop<%f&&mass>=%f&&pt<=1",acop_cut,mass_cut),20,0,acop_cut);
   ...
   MyCanvas mc1("mass_SS","Dielectron Invariant Mass [GeV]", "Entries / (2 GeV)", W, H);
   mc1.SetLogy(false);
   mc1.SetYRange(0.1,9000);
   mc1.SetRatioRange(0.1,1.9);
   mc1.SetLegendPosition(0.3,0.68,0.6,0.85);
   ...
   MyCanvas mc2("deltapt_SS","Dielectron #Delta p_{T} [GeV]", "Entries / (1GeV)", W, H);
   ...
   MyCanvas mc3("rap_SS","Dielectron y", "Entries / (0.2)", W, H);
   // mc3.SetLogy(false);
   ...
   MyCanvas mc4("pt_SS","Dielectron p_{T} [GeV]", "Entries / (0.5 GeV)", W, H);
   // mc4.SetLogy(false);
   mc4.SetYRange(0.,2000);
   mc4.SetRatioRange(0.1,1.9);
   mc4.SetLegendPosition(0.3,0.68,0.6,0.85);
   ...
   MyCanvas mc5("acop_SS","e^{+}e^{-} acoplanarity A", Form("Entries / (%.4f)",acop_cut/20.), W, H);
   mc5.SetLogy(false);
   mc5.SetYRange(2,9000);
   mc5.SetRatioRange(0.1,1.9);
   mc5.SetLegendPosition(0.3,0.68,0.6,0.85);
