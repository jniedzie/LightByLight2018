...
// name          x label           y label    logY     xMin    xMax    yMax
  {"pair_m"     , "m (GeV/c)"     , "Entries", true   , 0.0   ,  90.  , 1.0   , 1 },
  {"pair_pt"    , "p_{T} (GeV/c)" , "Entries", false  , 0.0   , 0.4   , 0.2   , 1 },
  {"pair_pz"    , "p_{z} (GeV/c)" , "Entries", true   , 0.0   , 1000  , 1.0   , 1 },
  {"pair_y"     , "y"             , "Entries", false  , -10.0 , 10.0  , 0.09  , 1 },
  {"pair_aco"   , "A_{#phi}"      , "Entries", true   , 0.0   , 1.0   , 0.5   , 1 },
};

vector<histParams> singleHistParams = {
// name          x label           y label    logY     xMin    xMax    yMax
  {"single_pt"  , "p_{T} (GeV/c)" , "Entries", true   , 0.0   , 50    , 0.5   , 5 },
  {"single_pz"  , "p_{z} (GeV/c)" , "Entries", true   , 0.0   , 1000  , 1.0   , 5 },
  {"single_y"   , "y"             , "Entries", false  , -10.0 , 10.0  , 0.04  , 1 },
  {"single_eta" , "#eta"          , "Entries", false  , -10.0 , 10.0  , 0.04  , 1 },
  {"single_phi" , "#phi"          , "Entries", false  , -4.0  , -4.0  , 0.02  , 1 },
};

TH1D* DrawPlot(TFile *inFile, histParams params, int color, bool firstFile)
{
  auto &[histName, xTitle, yTitle, logY, xMin, xMax, yMax, rebin] = params;
