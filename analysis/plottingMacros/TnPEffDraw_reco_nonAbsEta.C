...
// Location of the files
const char* fDataName = "tnp_Ana_Data_EleRecoID_singleEG3_pt2p0_eta2p2_invmass5_pairPt1_nonAbsEta.root";
const char* fMCName = "tnp_Ana_MC_EleRecoID_singleEG3_pt2p0_eta2p2_invmass5_pairPt1_nonAbsEta.root";
...
TString absetaVar("eta");
ofstream file_sfs("correction_functions_electronRecoID.txt");
...
void TnPEffDraw_reco_nonAbsEta() {
  ...
  {  //looping over barrel and endcap region to plot the efficiency as a function of pt for MC and data
     ComPt0.push_back(plotEff_1bin(daPtData0[i], 1, "pt"));
     ComPt1.push_back(plotEff_1bin(daPtData1[i], 1, "pt"));
  }

  //gets RooDataset as a function of eta from  tnpQED/elereco_abseta/fit_eff
  ...
  //plots the graph as a function of eta
  ...
  //get the RooDataset from tnpQED/elereco_1bin/fit_eff
  ...
  // gets just one number of efficiency as a function of eta (integrated efficiency)
  ...
  TGraphAsymmErrors* Com0Pt1 = plotEff_1bin(daPtData1Bin0,0,absetaVar);
   ...
   //get the RooDataset from tnpQED/elereco_EBEE/fit_eff 
  ...
  //gives the efficiency numbers in barrel and endcap region
  ...
  TH1F *hPad1 = new TH1F("hPad1",";#eta^{e} ;Single ele. Efficiency",10,-2.4,2.4);
...
// #endif // ifdef MUIDTRG
  ...
  // print the scale factors to file
  file_sfs<< "Scale factor as a function of eta " << endl;
  file_sfs << "// etamin etamax SF SF_uncert" << endl;
...
TGraphAsymmErrors *plotEff_1bin(RooDataSet *a, int aa, const char* varx){
  ...
  cout<<"Variable:"<< varx << "  NBins : "<<nbins<<endl;
