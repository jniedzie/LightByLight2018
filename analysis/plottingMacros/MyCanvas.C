...
public:
					  ...
					  TString DrawOp1 = "EP", TString DrawOp2 = "EPSAME", TString DrawOp_ref = "EPSAME")
  {
    ...
    // -- ensure additional space at the bottom side for ratio plot -- //
    //TopPad->SetBottomMargin(0.00); //0.32
    //TopPad->SetRightMargin(gStyle->GetPadRightMargin()/0.74);
    ...
    //h2c->SetFillStyle(3325);
    h2c->SetMarkerSize(0);
    h2c->Draw("E2 same");
    //h1->Draw("AXISSAME");
    TH1D *hc_ref = (TH1D*) h_ref->Clone();
    hc_ref->SetFillColor(kBlue+2);
    hc_ref->SetFillStyle(3004);
    //hc_ref->SetFillStyle(3352);
    hc_ref->SetMarkerSize(0);
    hc_ref->Draw("E2 same");
    h1->Draw(DrawOp1+"SAME");
    ...
    h2->SetMarkerSize(1);
    h2->SetMarkerStyle(20);
    if( isFill2 == kTRUE ) h2->SetFillColorAlpha(color2,0.35);
    else h2->SetFillColorAlpha(kWhite, 0);
    ...
    if( isFill_ref == kTRUE ) h_ref->SetFillColorAlpha(color_ref,0.35);
   ...
   c->cd();
    ...
    // -- Make Ratio plot with 2nd MC & Draw it -- //
		h_ratio2 = (TH1D*)h1->Clone();
		h_ratio2->SetName("h_ratio2");
		h_ref->Sumw2();
      TH1D *href_noerr = (TH1D*) h_ref->Clone();
      for (int i=0; i<=href_noerr->GetNbinsX()+1; i++) href_noerr->SetBinError(i,0);
		h_ratio2->Divide(h1, href_noerr);
		h_ratio2->Draw("EP SAME");

      // -- Make ratio plot for the h2 error
      TH1D *href_unit = (TH1D*) h_ref->Clone();
      for (int i=0; i<=href_unit->GetNbinsX()+1; i++) 
         if (href_unit->GetBinContent(i)!=0) {
            href_unit->SetBinError(i,href_unit->GetBinError(i)/href_unit->GetBinContent(i));
            href_unit->SetBinContent(i,1);
         }
      href_unit->SetFillColor(kBlack);
      href_unit->SetFillStyle(3004);
      href_unit->SetMarkerSize(0);
      href_unit->Draw("E2 same");
    ...
    //h_ratio1->GetXaxis()->SetLabelSize(0.15);
      h_ratio1->GetXaxis()->SetLabelSize(gTextSize2/0.3);
      h_ratio1->GetXaxis()->SetNdivisions(509);
      h_ratio1->GetXaxis()->SetTickLength(gStyle->GetTickLength()/0.35);
    ...
    // -- Y-axis Setting -- //
		...
		h_ratio1->GetYaxis()->SetTitleOffset( gYoffset * 0.33 );//0.4
		h_ratio1->GetYaxis()->SetTitleSize(gTextSize1/0.3);
		h_ratio1->GetYaxis()->SetLabelSize(gTextSize2/0.3);
		h_ratio1->GetYaxis()->SetRangeUser( RatioStandard-1+LowerEdge_Ratio, RatioStandard-1+UpperEdge_Ratio );
