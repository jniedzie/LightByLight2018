...
then
  ...
  outputPath="${basePath}/analysis_ruchi/electronID/data${suffix}"
elif [ $2 -eq 1 ]
then
...
#  inputPath="${basePath}/mc_qed/ntuples_superchic_1034/ntuples_sc_1034/ntuples_sc_1034/191113_105005/0000/HiForestAOD_LbyL_full_sample_lbyl_reco_${1}.root"
  ...
  outputPath="${basePath}/analysis_ruchi/electronID/mc_qed_sc${suffix}"
elif [ $2 -eq 2 ]
then
  ...
  outputPath="${basePath}/analysis_ruchi/electronID/mc_qed_sl${suffix}"
fi


mkdir -p $outputPath
output="${outputPath}/eleID_plots_QED_${1}.root"
...
then
  ...
  /afs/cern.ch/work/r/rchudasa/private/LightByLight2018/analysis/prepareElectronIDplots $configPath $inputPath $output $sampleName
fi
