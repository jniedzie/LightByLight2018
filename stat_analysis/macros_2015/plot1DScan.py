...
for other in other_scans:
    ...
    if args.hide_envelope:
        ...
        other['graph'].Draw('SAME') # redraw this
...
textfit = '#sigma = %.0f{}^{#plus %.0f}_{#minus %.0f} nb' % (
    val_nom[0], val_nom[1], abs(val_nom[2]))
if args.upper_cl:
    textfit = '%s < %.2f (%i%% CL)' % (
...
plot.DrawCMSLogo(pads[0], 'CMS','Supplementary',
...
plot.DrawTitle(pads[0], '#lower[-0.1]{PbPb (391 {#mu}b^{-1}, #sqrt{s_{NN}} = 5.02 TeV)}', 3)
