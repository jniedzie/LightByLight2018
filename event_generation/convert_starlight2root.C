...
void convert_starlight2root(const char *infile="slight.out",
                            const char *fileOut="starlight_histos.root",
                            const double sigma_mub=1.)
{
  ...
  Int_t nbins = 100;
  Double_t ptmin=0.0, ptmax=2 ; // GeV
  ...
  TH1F * y_pair_distrib = new TH1F ("pair_y", "dN/dy", nbins, ymin, ymax);
  TH1F * m_pair_distrib = new TH1F ("pair_m", "dN/dm", nbins, mmin, mmax);
  TH1F * pt_pair_distrib = new TH1F ("pair_pt","dN/dp_{T}",nbins,ptmin,ptmax);
//  TH1F * aco_pair_distrib = new TH1F("aco_pair_distrib","dN/d#phi",nbins,-Pi,Pi);
  TH1F * aco_pair_distrib = new TH1F("pair_aco","dN/dA_{#phi}",nbins,0,1);
  TH1F * theta_pair_distrib = new TH1F("theta_pair_distrib","dN/d#theta",nbins,0,Pi);
  TH1F * pz_pair_distrib = new TH1F ("pair_pz","dN/dp_{L}",nbins,plmin,plmax);
  
  TH1F * y_single = new TH1F ("single_y", "dN/dy", nbins, ymin, ymax);
  TH1F * eta_single = new TH1F ("single_eta", "dN/d#eta", nbins, etamin, etamax);
  TH1F * pt_single = new TH1F ("single_pt","dN/dp_{T}",nbins*100,ptmin,ptmax*100.);
  TH1F * pz_single = new TH1F ("single_pz","dN/dp_{L}",nbins*10,plmin,plmax);
  TH1F * phi_single = new TH1F ("single_phi","dN/d#phi",nbins,-4,4);
  ...
  pt_pair_distrib->GetYaxis()->SetTitle("Entries");

  pz_pair_distrib->GetXaxis()->SetTitle("p_{L} #left[ GeV/c #right]");
  pz_pair_distrib->GetYaxis()->SetTitle("Entries");

  aco_pair_distrib->GetXaxis()->SetTitle("A_{#phi}");
  ...
  pt_single->GetYaxis()->SetTitle("Entries");

  pz_single->GetXaxis()->SetTitle("p_{L} single");
  pz_single->GetYaxis()->SetTitle("Entries");

  phi_single->GetXaxis()->SetTitle("$phi single");
  phi_single->GetYaxis()->SetTitle("Entries");
...
//      double Dphi=TMath::Abs(Pi-TMath::Abs(particle2.Phi()-particle1.Phi())); // acoplanarity
      ...
      double Dphi=1 - TMath::Abs(TMath::Abs(particle2.Phi()-particle1.Phi()))/Pi; // acoplanarity
...
//      aco_pair_distrib->Fill(particlesum.Phi());
